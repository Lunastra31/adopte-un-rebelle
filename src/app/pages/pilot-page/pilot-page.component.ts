...
import {
  ...
  ChangepilotstatusmodalModel,
} from 'src/app/components/modals/changepilotstatusmodal/changepilotstatusmodal.component';
...
export class PilotPageComponent implements OnInit {
  ...
  redirectToChangePilotStatus(pilotChangedStatus: Pilot) {
    const dialogData = new ChangepilotstatusmodalModel(
      'Changer le status du pilote ',
      pilotChangedStatus
    );
    this.dialog
      .open(ChangepilotstatusmodalComponent, {
        maxWidth: '1000px',
        data: dialogData,
      })
      .afterClosed()
  ...
  filterPilots() {
    ...
    if (this.selectedFilter === 'APPRENTI') {
      this.pilots = this.pilots.filter((pilot) => pilot.trainee === true);
    } else {
